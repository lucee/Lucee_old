...
public abstract class Function extends StatementBaseNoFinal implements Opcodes, IFunction,HasBody {
	...
	public Function(Root root,String name,int access,int modifier,String returnType,Body body,Position start, Position end) {
		...
		if(!StringUtil.isEmpty(returnType))
			this.returnType=body.getFactory().createLitString(returnType);
		else
			this.returnType=body.getFactory().createLitString("any");
		this.body=body;
		body.setParent(this);
	...
	public Function(Root root,Expression name,Expression returnType,Expression returnFormat,Expression output,Expression bufferOutput,
